...
//    int[] DFS(int node, int player, int[]  pieceslocation)
//    {
//        boolean[] isVisited = new boolean [vno];
//        pieceslocation= DFSrecursion(node, player, isVisited, pieceslocation);
//
//
//        return pieceslocation;
//    }

    int DFS(int node,int player, boolean Visited[], int [] pieceslocation)
    {
         int piecesfound=0;
        Visited[node] = true;
        ...
        for (Integer adjNode : getAd(node))
        {


            if(!Visited[adjNode])
            {
                System.out.println("Node: " + adjNode + "pieces "+ pieceslocation[adjNode]);
                piecesfound+=pieceslocation[adjNode];
                pieceslocation[adjNode]=0;
                //System.out.println("Adj: " + adjNo
                piecesfound+= DFS(adjNode,player, Visited, pieceslocation);
    ...
    public static void main(String[] args) {
        ...
        for(int k=0; k<F; k++) {
            ...
            boolean visitedcities[]= new boolean [C];
            int p= map.DFS(Cy, Fy, visitedcities, piecelocations);
        }
